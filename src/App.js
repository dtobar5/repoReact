...
const App = () => {
  ...
  React.useEffect(() => {
    ...
    fetch(url).then((response) => {
      ...
      .catch((error) => console.log(`se pudrio todo,fue un error ${error.status}`));
  ...
  return (
    ...
    <div className="App" >
      ...
      <>
        <br></br>
        <button onClick={postProducto} className="fuente">Cargar nuevo producto</button><br></br>
        {cargando && <p className="fuente">Cargando...</p>}
        {error && <p className="fuente">Ha habdo un error: {error.status} {error.statusText}</p>}
        {dat?.map((product) => {
          return (
            <div className="cardContainer">
              <ul>
